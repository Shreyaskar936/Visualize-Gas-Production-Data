...
def plot():
    ...
    if filtered_data.empty:
        ...
        ax.text(0.5, 0.5, 'No Records\nAvailable', fontsize=45, ha='center', va='center', color='gray', fontname='Arial Rounded MT Bold')
        ax.axis('off')
    else:
        ...
        if selected_option == 0:
            ax.set_title(f'Well-wise Production (Cumulative) for Field: {selected_field}, Year: {selected_year}, Month: {selected_month}', fontsize=20, fontname='Arial Rounded MT Bold')
        else:
            ax.set_title(f'Well-wise Production (Cumulative) for Sand: {selected_layer}, Year: {selected_year}, Month: {selected_month}', fontsize=20, fontname='Arial Rounded MT Bold')
...
Label(f3, text="----------------------------   ONGC MONTHLY GAS PRODUCTION TRIPURA ASSET   ----------------------------",
      bg="#38070f", fg="white", font=("Cooper BT", 30, "bold")).pack(padx=20, pady=20)
...
Radiobutton(f1, variable=select, value=0, text="Field Wise:",
            bg="#e3d1d4", fg="#45191d", font=("Verdana", 14, "bold"),
            relief=RAISED, padx=9, pady=7, command=state).grid(row=0, column=0, padx=(34, 10), pady=(20, 10))

Radiobutton(f1, variable=select, value=1, text="Sand Wise:",
            bg="#e3d1d4", fg="#45191d", font=("Verdana", 14, "bold"),
            relief=RAISED, padx=5, pady=6, command=state).grid(row=0, column=1, padx=(25, 30), pady=(25, 10), sticky=W)
...
Label(f1, text="Year:",
      bg="#e3d1d4", fg="#45191d", font=("Verdana", 14, "bold"),
      relief=RAISED).grid(row=2, column=0, padx=(28, 2), pady=17, ipadx=19, ipady=10)
...
Label(f1, text="Month:",
      bg="#e3d1d4", fg="#45191d", font=("Verdana", 14, "bold"),
      relief=RAISED).grid(row=3, column=0, pady=7, padx=(30, 2), ipadx=10, ipady=10)

month = ttk.Combobox(f1, values=data_months, font=("Verdana", 13, "bold"), width=15)
month.grid(row=3, column=1, pady=7, sticky=W)
month.current(0)

Button(f1, text="SUBMIT", command=plot, font=("Verdana", 14, "bold"), bg="#820314", fg="#ffffff",
       activebackground="#7661ff", activeforeground="#ffffff", relief=RAISED, bd=3,
       overrelief=SUNKEN).grid(row=4, column=1, pady=7, ipadx=0, ipady=6, sticky=W)

Button(f1, text="CLEAR", command=clear, font=("Verdana", 14, "bold"), bg="#820314", fg="#ffffff",
       activebackground="#7661ff", activeforeground="#ffffff", relief=RAISED, bd=3,
       overrelief=SUNKEN).grid(row=4, column=1, pady=7, ipady=6, sticky=E, padx=19)

img = Image.open(r"Project1\ongc.png")
